...
@Component({
  selector: 'app-rutas-escanear',
  templateUrl: './rutas-escanear.page.html',
  styleUrls: ['./rutas-escanear.page.scss'],
})
export class RutasEscanearPage implements OnInit {
  ...
  constructor(private zbar: ZBar, private eleccionesService: EleccionesService, 
    private plataform: Platform, private nativeAudio: NativeAudio, private modal: ModalController) {
  ...
  Escanear(){
    ...
    this.zbar.scan(this.zbarOptions).then(result => {
      ...
      this.zbarOptions.text_instructions = this.contador+" de "+this.max+" cajas";
